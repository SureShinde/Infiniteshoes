	...
	$.mouseOverZoomTiles = function(options){
		...
		var getRelDim = function(w, h){
			if (typeof w == 'string' && w.length > 1 && w != 'auto'){
				...
				if (relDivW.length){w = relDivW.outerWidth();}
				if (tempPar[1]){w += parseInt(tempPar[1]);}
			}

			if (typeof h == 'string' && h.length > 1 && h != 'auto'){
				var tempPar = h.split('|'), relDivH = $(tempPar[0]);
				if (relDivH.length){h = relDivH.outerHeight();}
				if (h && tempPar[1]){h += parseInt(tempPar[1]);}
			}
			return [w, h];
		};
			...
			if (mediaQueryFullWidthRequest){
				...
				if (isInt(op.zoomHeight)){
					...
					if (isInt(relDim[1])){
						...
						if (targetHeight > autoH){targetHeight = autoH;}
					}else{
						// auto
						targetHeight = autoH;
					}
		...
		var triggerHide = function(t){
			...
			if (!isInt(t)){t = op.fadeOutTimeout;}
			if (removeHoverTimeout){clearTimeout(removeHoverTimeout);}
