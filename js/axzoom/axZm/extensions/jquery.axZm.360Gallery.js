...
* If you have done something cool you can send us your version and after review we will update the download package.
*/
...
;(function($){
	...
	$.axZm360Gallery = function(opt){
		...
		var loadGalleryCallback = function(){
			...
			if (onSpinPreloadEndOnce){return;} // exit if already executed
		...
		var showVideo = function(lnk, kind){
			var videFrame = $('<iframe />')
			.addClass('azVideoIframe')
			.data('lnk', lnk)
			.attr({
				src: op.videoUrl[kind]+lnk+'?'+$.param(op.videoSettings[kind]),
				width: '100%',
				height: '100%',
				frameborder: 0,
				allowfullscreen: ' '
		...
		var loadFirst = function(){
			...
			// Mixed gallery
				...
				if (firstToLoadType == 'image360'){
					...
					showVideo(galleryData.firstToLoad[1], firstToLoadType);
					currentZoomMod = 'video';
					loadGalleryCallback();
					return;
				}
	...
	$.axZm360Gallery.getParam = function(obj){
		var ref = $('#'+obj);
		if (ref.length > 0){
			return ref.data('aZ');
		}
