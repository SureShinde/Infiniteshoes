...
class ParadoxLabs_TokenBase_Model_Method extends Mage_Payment_Model_Method_Cc
{
	...
	{
		// Skip zero total check (we want to allow)
		$checksBitMask &= ~self::CHECK_ZERO_TOTAL;
	...
	/**
	 * Allow 3D Secure validation only if enabled and we're not using a stored card
	 */
	public function getIsCentinelValidationEnabled()
	{
		$parent = parent::getIsCentinelValidationEnabled();

		// Don't allow validation if we're using a stored card.
		if( $parent === true
			&& $this->getInfoInstance()->getTokenbaseId() != ''
			&& $this->getInfoInstance()->getCcNumber() == '' ) {
			// If we're here from the admin panel, throw a friendly error message.
			$request = Mage::app()->getRequest();
			if( $request->getRouteName() == 'adminhtml'
				&& $request->getControllerName() == 'centinel_index'
				&& $request->getActionName() == 'validatepaymentdata' ) {
				throw new Mage_Core_Exception(
					Mage::helper('tokenbase')->__('3D Secure Card Validation is not available for stored credit cards.')
				);
			}
	...
	/**
	 * If enabled and relevant, set Centinal 3D Secure params on the gateway for the current transaction.
	 */
	protected function _setCentinelParams( Varien_Object $payment )
	{
		if( $this->getIsCentinelValidationEnabled() && $this->_paymentContainsCard( $payment ) ) {
			$centinel = $this->getCentinelValidator()->exportCmpiData( new Varien_Object() );
			$this->gateway()->setParameter( 'centinelAuthIndicator', $centinel->getData( Mage_Centinel_Model_Service::CMPI_ECI ) );
			$this->gateway()->setParameter( 'centinelAuthValue', $centinel->getData( Mage_Centinel_Model_Service::CMPI_CAVV ) );
		}
