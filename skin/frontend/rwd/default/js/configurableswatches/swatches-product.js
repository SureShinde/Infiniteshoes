...
Product.ConfigurableSwatches.prototype = {
    ...
    onOptionClick: function(attr) {
        ...
        /*
         * CBI DEV - custom khi click tren size
         */
        if(opt.attr['code']=='size'){
            clickOnSize(opt.id);
            return false;
        }else{
            // CBI DEV - remove selected cho size khi click tren color
            removeSizeSelected();
        }
        /*var enable = opt._f.enabled;
        if(enable==false && opt.attr['code']=='size'){
            return false;
        }*/
        ...
        if ((this._O.selectFirstOption && !this._F.firstOptionSelected) ||
            ...
            /**
             * CBI DEV - custom de ko change hinh khi click vao size
             **/
            if(opt.attr['code']!='size'){
                this.updateSelect(attr);
                this._F.firstOptionSelected = true;
...
function removeSizeSelected(){
    jQuery("#configurable_swatch_size > li").removeClass("selected");
    jQuery("#configurable_swatch_size").parent().find('select').val('');
}

function clickOnSize(id){
    if(jQuery("#option"+id).hasClass('not-available')){
        return false;
    }else{
        jQuery("#configurable_swatch_size li").removeClass('selected');
        jQuery("#option"+id).addClass('selected');
        jQuery("#configurable_swatch_size").parent().find('select').val(id);
    }
}